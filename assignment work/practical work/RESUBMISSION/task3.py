#!/usr/bin/python
import sqlite3
db = sqlite3.connect('orinoco.db', isolation_level= None)
...
get_payment = "SELECT\
                  ...
                  FROM shoppers s\
                        INNER JOIN\
                  shopper_orders so ON so.shopper_id = s.shopper_id\
                        INNER JOIN\
                  shopper_payment_cards spc ON spc.payment_card_id = so.payment_card_id\
                  WHERE s.shopper_id = ?;"                                             #retrieves the payment details for a shopper, when provided the shopper_id

get_delivery_address = "SELECT sda.delivery_address_id,\
                            sda.delivery_address_line_1,\
                            sda.delivery_address_line_2,\
                            sda.delivery_address_line_3,\
                            sda.delivery_county,\
                            sda.delivery_post_code\
                        FROM shopper_delivery_addresses sda\
                            INNER JOIN\
                            shopper_orders so ON so.delivery_address_id = sda.delivery_address_id\
                        WHERE shopper_id = ?;"                                         #retrieves the shopper's delivery addresses, when provided a shopper_id

order_confirm = "INSERT INTO shopper_orders(shopper_id,delivery_address_id,payment_card_id,order_date,order_status)\
                 VALUES(?,?,?,date('now'),'Placed')"                                   #inserts the necessary row into the shopper_orders table, to move an order from the basket to the shopper orders.
    ...
    if shopper_id.isdigit():                  #ensuring an actual digit has been entered, as anything else will not work.
        ...
        if menu_input == '1':
            ...
            if checkout_confirm == 'y':
                cursor.execute(get_delivery_address,[shopper_id])
                all_rows = cursor.fetchall()
                _display_options(all_rows,"delivery addresses","address")
                delivery_address = selected_option
                cursor.execute(get_payment,[shopper_id])
                all_rows = cursor.fetchall()
                _display_options(all_rows,"payment options","payment option")
                selected_payment = selected_option
                confirm_order = input("confirm purchase? (y/n)")
                if confirm_order == 'y':
                    cursor.execute(order_confirm,((shopper_id),(delivery_address),(selected_payment)))
